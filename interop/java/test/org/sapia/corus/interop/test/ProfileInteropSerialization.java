 ...
 * @version 1.0
 */
public class ProfileInteropSerialization implements Runnable {
  private static int  totalTestcase = 0;
  private static long startTime;
  private InteropSerializationTest _theTest;

  public ProfileInteropSerialization(InteropSerializationTest aTest) {
    _theTest = aTest;
  }

  public static void main(String[] args) {
    InteropSerializationTest aTest = new InteropSerializationTest("main");

    try {
      System.out.println("Starting profiling of Interop serialization...");
      ...
      for (int i = 0; i < 2; i++) {
        new Thread(new ProfileInteropSerialization(aTest), "Test-" + (i + 1)).start();
      }
