...
/**
 * This extension emits status gathered from the Corus server VM. The Output format
 * is XML. This extension can be accessed with an URL similar as the following one:
 * <p>
 * <pre>http://localhost:33000/jmx</pre>
 * 
 * @author yduchesne
 *
 */
public class JmxExtension implements HttpExtension{
  ...
  private void doProcess(HttpContext ctx) throws Exception{
    ...
    attribute("runtime.startTime",  
        new Long(runtime.getStartTime()), ps);
    attribute("runtime.startDate",  
        new Date(runtime.getStartTime()), ps);    
    attribute("runtime.upTime",  
        new Long(runtime.getUptime()), ps);
    
    /////// MEMORY

    MemoryMXBean mem = ManagementFactory.getMemoryMXBean();
    
    attribute("mem.pendingFinalization.count", 
        new Integer(mem.getObjectPendingFinalizationCount()), ps);
    
    // heap
    
    attribute("mem.heap.init", 
        new Long(mem.getHeapMemoryUsage().getInit()), ps);
    attribute("mem.heap.committed", 
        new Long(mem.getHeapMemoryUsage().getCommitted()), ps);    
    attribute("mem.heap.max", 
        new Long(mem.getHeapMemoryUsage().getMax()), ps);    
    attribute("mem.heap.used", 
        new Long(mem.getHeapMemoryUsage().getUsed()), ps);    
    
    // non-heap
    
    attribute("mem.non-heap.init", 
        new Long(mem.getNonHeapMemoryUsage().getInit()), ps);
    attribute("mem.non-heap.committed", 
        new Long(mem.getNonHeapMemoryUsage().getCommitted()), ps);    
    attribute("mem.non-heap.max", 
        new Long(mem.getNonHeapMemoryUsage().getMax()), ps);
    attribute("mem.non-heap.used", 
        new Long(mem.getNonHeapMemoryUsage().getUsed()), ps);    
    ...
    attribute("threads.count",  
        new Integer(threads.getThreadCount()), ps);    
    attribute("threads.daemon.count",  
        new Integer(threads.getDaemonThreadCount()), ps);
    attribute("threads.peak.count",  
        new Integer(threads.getPeakThreadCount()), ps);    
    attribute("threads.totalStarted.count",  
        new Long(threads.getTotalStartedThreadCount()), ps);
    
    /////// CLASSLOADING

    ClassLoadingMXBean classes = ManagementFactory.getClassLoadingMXBean();

    attribute("classes.loadedCount",  
        new Integer(classes.getLoadedClassCount()), ps);    
    attribute("class.totalLoadedCount",  
        new Long(classes.getTotalLoadedClassCount()), ps);
    attribute("classes.unloadedCount",  
        new Long(classes.getUnloadedClassCount()), ps);    
    ...
    attribute("os.processors",  
        new Integer(os.getAvailableProcessors()), ps);    
