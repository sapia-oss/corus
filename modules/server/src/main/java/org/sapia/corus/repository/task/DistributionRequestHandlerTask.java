...
public class DistributionRequestHandlerTask extends ArtifactRequestHandlerTaskSupport {
   ...
   */
  public DistributionRequestHandlerTask(final File distFile, List<Endpoint> targets) {
    super(distFile, targets, new Function<DeploymentMetadata, Boolean>() {
      @Override
      public DeploymentMetadata call(Boolean clustered) {
        return new DistributionDeploymentMetadata(distFile.getName(), distFile.length(), clustered);
      }
    });
  }
