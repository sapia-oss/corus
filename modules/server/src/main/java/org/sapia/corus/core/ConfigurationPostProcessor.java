...
/**
 * An instance of this class replaces property variables within the spring
 * config files, using the values supplied as part of the corus configuration,
 * or the ones stored in the {@link Configurator} - the latter as precedence.
 * 
 * @author yduchesne
 * 
 */
    ...
    for (String name : factory.getBeanDefinitionNames()) {
      ...
      for (int i = 0; i < vals.length; i++) {
        ...
        if (toReplace != null) {
          ...
          PropertyValue newVal = new PropertyValue(oldVal.getName(), subs.replace(toReplace));
