...
public class FileDeploymentHandler implements DeploymentHandler {
  ...
  @Override
  public ProgressQueue completeDeployment(DeploymentMetadata meta,
      File uploadedFile) {
  ...
  public File getDestFile(DeploymentMetadata meta) {
    ...
    if (fileMeta.getDirName() == null) {
      return FilePath.newInstance()
          .addDir(configuration.getUploadDir())
          .setRelativeFile(meta.getFileName())
          .createFile();
    } else {
      return FilePath.newInstance()
          .addDir(fileMeta.getDirName())
          .setRelativeFile(meta.getFileName())
          .createFile();
    }
  }
