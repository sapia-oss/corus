...
public class ShellScriptListResponseHandlerTask extends RunnableTask {
  ...
  public void run() {
    ...
    context().debug(String.format(
        "Got script list (%s) from %s", 
        scriptsRes.getScripts(), 
        scriptsRes.getEndpoint()));

    if (!toReceive.isEmpty()) {
      context().debug("Sending shell script deployment request");
      EventChannel          channel = cluster.getEventChannel();
      ShellScriptDeploymentRequest request = new ShellScriptDeploymentRequest(
          context().getServerContext().getCorusHost().getEndpoint(), 
