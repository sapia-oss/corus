...
/**
 * Cleans up the process directory and removes it from the 
 * active process list.
 * 
 * @author yduchesne
 *
...
public class CleanupProcessTask extends Task<Void, Process>{
  ...
  @Override
  public Void execute(TaskExecutionContext ctx, Process proc)
      throws Throwable {
    ...
    if(proc.getStatus() != LifeCycleStatus.KILL_CONFIRMED){
      throw new IllegalStateException(
          String.format("Cannot kill process %s; current status is %s, expected: %s", 
          proc, proc.getStatus(), 
