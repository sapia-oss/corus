 ...
 * org.apache.cocoon.matching.helpers.WildcardHelper CLASS. THIS ALLOWS NOT
 * HAVING DEPENCIES ON COCOON AT COMPILE-TIME/RUNTIME, IF COCOON IS NOT USED.
 * <p>
...
public class UriPatternHelper {
   ...
   * </ul>
   * When more than two '*' characters, not separated by another character, are
   * found their value is considered as '**' (MATCH_PATH). <br>
   * The array is always terminated by a special value (MATCH_END). <br>
   * All MATCH* values are less than zero, while normal characters are equal or
   * greater.
   * 
   * @param data
   ...
   * <code>Stack</code>.
   */
  static boolean match(Map map, String data, int[] expr) throws NullPointerException {
    if (map == null)
