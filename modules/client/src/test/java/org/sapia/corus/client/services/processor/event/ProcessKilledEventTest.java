...
public class ProcessKilledEventTest {
  ...
  public void testGetLevel_with_requestor_not_server() {
    assertThat(event(ProcessTerminationRequestor.KILL_REQUESTOR_PROCESS, false).getLevel()).isEqualTo(EventLevel.INFO);
  }
  ...
  public void testToEventLog() {
    ...
    assertThat(log.getType()).isEqualTo(ProcessKilledEvent.class.getSimpleName());
  ...
  private ProcessKilledEvent event() {
    return event(ProcessTerminationRequestor.KILL_REQUESTOR_SERVER, true);
  }

  private ProcessKilledEvent event(ProcessTerminationRequestor req, boolean wasRestarted) {
    return new ProcessKilledEvent(req, process, wasRestarted);
  }
}
