...
public class Match extends NoOptionCommand {
  ...
  public void doExecute(CliContext ctx, Reader script, Arg patternArg, CmdLine remaining) throws IOException, AbortException, InputException {
    ...
    if (script != null && FacadeInvocationContext.get() instanceof List) {
      ...
      if (!results.isEmpty()) {
        ...
        for (Object r: results) {
          if (r instanceof Result) {
            ...
            if ((result.getData() != null && result.getData().getClass().isArray() && Array.getLength(result.getData()) > 0)
                || (result.getData() instanceof Collection<?> && ((Collection<?>) result.getData()).size() > 0)
                || (result.getData() != null && !result.getData().getClass().isArray() && !(result.getData() instanceof Collection<?>))) {
        ...
        if (!selectedHosts.isEmpty()) {
          ctx.getConsole().println("Positive match, executing script...");
        ...
        ctx.getConsole().println("No match occurred: script will not be executed");
      }
    }
  }
