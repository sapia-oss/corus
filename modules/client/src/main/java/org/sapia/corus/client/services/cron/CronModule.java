...
public interface CronModule extends java.rmi.Remote, Module {
   ...
   * 
   * @param info a {@link CronJobInfo} corresponding to a Cron job to add.
   * @throws InvalidTimeException if the time that is specified is invalid.
   * @throws DuplicateScheduleException if a schedule already exists for the given job.
   * @throws ProcessConfigurationNotFoundException if no process matches the given job.
   * @throws CorusException if an undefined error occurs.
   */
  ...
  /**
   * @return the {@link List} of {@link CronJobInfo} instances that this instance holds.
   */
