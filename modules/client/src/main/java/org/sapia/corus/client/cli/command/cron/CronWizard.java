...
public class CronWizard {
  ...
  public void execute(CmdLine cmd, CliContext ctx) throws AbortException, InputException {
    ...
    try {
      ...
      throw new AbortException("Error entering data", e);
    }
  ...
  private void hour(CronJobInfo info, CliContext ctx) throws IOException, AbortException, InputException {
  ...
  private void minutes(CronJobInfo info, CliContext ctx) throws IOException, AbortException, InputException {
  ...
  private void dayOfWeek(CronJobInfo info, CliContext ctx) throws IOException, AbortException, InputException {
    ...
    ctx.getConsole().prompt();
    info.setDayOfWeek(assertRange(process(ctx.getConsole().in().readLine()), 1, 7));
  }

  private void dayOfMonth(CronJobInfo info, CliContext ctx) throws IOException, AbortException, InputException {
    ...
    ctx.getConsole().prompt();
    info.setDayOfMonth(assertRange(process(ctx.getConsole().in().readLine()), 1, 31));
  }

  private void month(CronJobInfo info, CliContext ctx) throws IOException, AbortException, InputException {
  ...
  private void year(CronJobInfo info, CliContext ctx) throws IOException, AbortException, InputException {
  ...
  private static int assertRange(int value, int low, int high) throws InputException {
    if (value < 0) {
      return value;
    }

    if ((value < low) || (value > high)) {
      throw new InputException("Value must be between " + low + " and " + high + " inclusively");
    } else {
      return value;
    }
  }
  ...
  private int process(String input) throws AbortException, InputException {
    if ((input == null) || (input.length() == 0)) {
      throw new AbortException("No input provided");
    }
