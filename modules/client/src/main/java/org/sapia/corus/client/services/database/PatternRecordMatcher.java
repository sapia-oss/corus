...
/**
 * An instance of this class matches field values of a {@link Record} based on a
 * predefined patterns. It also takes custom {@link FieldMatcher}s (see
 * {@link #add(String, FieldMatcher)}).
 * 
...
public class PatternRecordMatcher<V> implements RecordMatcher<V> {
   ...
   * 
   * @param dbMap
   *          a {@link DbMap} for which to create a new instance of this class.
   ...
   *          a field name.
   * @param matcher
   *          the matcher to use against the corresponding field's value.
   ...
   *          a field name.
   * @param pattern
   *          the pattern to use against the corresponding field's value.
   * @return this instance.
   */
  public PatternRecordMatcher<V> addPattern(String fieldName, String pattern) {
    if (pattern == null) {
      ...
      getMatchers(fieldName, true).add(new ArgFieldMatcher(ArgMatchers.parse(pattern)));
    }
    return this;
  }
  ...
  public class ArgFieldMatcher implements FieldMatcher {
    ...
    ArgFieldMatcher(ArgMatcher arg) {
      this.arg = arg;
    }
