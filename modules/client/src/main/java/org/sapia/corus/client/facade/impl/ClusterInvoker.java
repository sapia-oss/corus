...
public class ClusterInvoker<M> {
  ...
  /**
   * Performs a "lenient" invocation (only {@link RuntimeException}s will be
   * thrown; non-runtime exceptions will be wrapped in runtime exceptions).
   * 
   * @param results
   *          a {@link Result}s instance that will be filled with the results of
   *          the clustered invocation.
   * @param info
   *          a {@link ClusterInfo} instance.
   * 
   * @see InvocationDispatcher#invoke(Results, Class, Method, Object[],
   *      ClusterInfo)
   */
  ...
  /**
   * Performs a "lenient" invocation (only {@link RuntimeException}s will be
   * throw; non-runtime exceptions will be wrapped in runtime exceptions).
   * 
   * @param returnType
   ...
   * 
   * @see InvocationDispatcher#invoke(Class, Class, Method, Object[],
   *      ClusterInfo)
   */
   ...
   *          the interface whose methods should be invoked.
   * @return a dynamic proxy implementing the given interface.
   */
  public <T> T proxy(Class<T> moduleInterface) {
    return moduleInterface.cast(Proxy.newProxyInstance(Thread.currentThread().getContextClassLoader(), new Class[] { moduleInterface },
        new ClusterInvokerHandler()));
  }

  class ClusterInvokerHandler implements InvocationHandler {

    @Override
    public synchronized Object invoke(Object proxy, Method someMethod, Object[] someParams) throws Throwable {
      toInvoke = someMethod;
      params = someParams;
      return null;
    }
