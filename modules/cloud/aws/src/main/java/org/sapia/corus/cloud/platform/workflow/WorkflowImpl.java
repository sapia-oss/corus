  ...
  public WorkflowImpl(WorkflowLog log, List<WorkflowStep<C>> steps) {
    ...
    wrappers = Lists.transform(steps, new Function<WorkflowStep<C>, StepWrapper>() {
      ...
      public StepWrapper apply(WorkflowStep<C> s) {
        ...
        w.desc = new StepDescriptor(decamelizeName(s.getClass().getName()), s.getDescription());
        w.step = s;
        return w;
      }
    });
        ...
        if (w.step instanceof GuardedExecutionCapable) {
          ...
          for (Class<?> p : preReqs) {
            if (hasPrerequisite(runStepClasses, p)) {
              ...
              try {
                log.info("===========> Executing: %s (%s)", w.desc.getStepName(), w.desc.getStepDescription());
                ...
                log.warning("Error caught running step %: %s", w.desc.getStepName(), e.getMessage());
                log.verbose(e);
              }
              break;
            }
