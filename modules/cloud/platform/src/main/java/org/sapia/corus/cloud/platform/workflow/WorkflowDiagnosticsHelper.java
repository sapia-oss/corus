...
public class WorkflowDiagnosticsHelper {
  ...
  public void displayDiagnostics(WorkflowResult result) {
    ...
    if (result.getOutcome() == Outcome.FAILURE) {
      log.info("Execution FAILED - took approximately : " + result.getDuration().approximate().toLiteral());
    } else {
      log.info("Execution SUCCEEDED - took approximately : " + result.getDuration().approximate().toLiteral());
    }
    log.info("");
